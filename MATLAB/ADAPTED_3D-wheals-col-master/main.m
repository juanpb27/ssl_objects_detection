...
% a Global Surface Removal stage is carried out to obtain Sp and isolating 
...
% --------------------------------------------------------------------------------------
% get ROI
figure(1), imagesc(S.Z), axis image off; colorbar
[S.Z, rec] = imcrop;
S.X = imcrop(S.X,rec);
S.Y = imcrop(S.Y,rec);
% --------------------------------------------------------------------------------------
...
l = [30 70]; % mask limits
Mask = nan(size(Sp.Z));
Mask(l(1):end-l(2), l(1):end-l(1)) = 1;
% Zero padding...
Spz = Sp.Z.*Mask;
Spz(isnan(Spz)) = 0;
figure,imagesc(Spz); colorbar
% --------------------------------------------------------------------------------------
% Saving results
%save('mat/Spz.mat','Spz')
% --------------------------------------------------------------------------------------
% Gaussian Pyramid
g_levels = 3;
gauss_pyr = genPyr(Sp,'gauss',g_levels); % the Gaussian pyramid
% --------------------------------------------------------------------------------------
% Display pyramid levels
Spz_pyr = cell(1,g_levels);

for i = 1:g_levels
    Mask2 = nan(size(gauss_pyr{i}));
    Mask2(l(1):end-l(2), l(1):end-l(1)) = 1;
    Spz_pyr{i} = gauss_pyr{i}.*Mask2;
    Spz_pyr{i}(isnan(Spz_pyr{i})) = 0;
    figure, imagesc(Spz_pyr{i}), colorbar
    l = ceil(l/2);
end
% Saving results
save('python/data2.mat', 'Spz_pyr')
% --------------------------------------------------------------------------------------